...
public class AdministradoController {
	...
	public String savePaciente(HttpSession session,@Validated Paciente paciente,BindingResult bindingResult,String menstruacion,String med,String enf,Map<String,Object> model){
		if(session.getAttribute("idusuario")!=null){
			Administrador admin=administradorService.findByDni(session.getAttribute("idusuario").toString());
			if(admin.getRol().equalsIgnoreCase("Admin")){
				int encontrado=0;

				if(pacienteService.findByDni(paciente.getDni())!=null){
					encontrado=1;
				}

				if(tecnologoService.findByDni(paciente.getDni())!=null){
					encontrado=1;
				}

				if(administradorService.findByDni(paciente.getDni())!=null){
					encontrado=1;
				}
				if(bindingResult.hasErrors() || encontrado==1 ||(paciente.getMedicamento().equals("Si") && med.trim().length()==0) || (paciente.getEnfermedad().equals("Si") && enf.trim().length()==0)){
				...
				if(paciente.getSexo().equals("M")){
					...
					paciente.setMenstruacion(menstruacion);
				}	
	...
	public String updatePaciente(HttpSession session,@Validated Paciente paciente,BindingResult bindingResult,String menstruacion,String med,String enf,Map<String, Object> model){
		if(session.getAttribute("idusuario")!=null){
			Administrador admin=administradorService.findByDni(session.getAttribute("idusuario").toString());
			if(admin.getRol().equalsIgnoreCase("Admin")){
				med=med.trim();
	...
	public String verCita(HttpSession session,@PathVariable Integer id_cita, Map<String, Object> model) {
		if(session.getAttribute("idusuario")!=null){
			...
			if(admin.getRol().equalsIgnoreCase("Admin")){
				...
				List<DetalleCita> detalleCitas=detalleCitaService.findByCita(cita);
				model.put("cita", cita);
				model.put("resultados",resultados);
				model.put("detalleCitas", detalleCitas);
				return "administrador/verCita";
			}else{
				return "error/error";
			}
		}else{
	...
	@GetMapping("/reporteCitaPorPaciente")
	public String reporteCitaPorPaciente(HttpSession session,Map<String,Object> model) {
		if(session.getAttribute("idusuario")!=null){
			Administrador admin=administradorService.findByDni(session.getAttribute("idusuario").toString());
			if(admin.getRol().equalsIgnoreCase("Admin")){
				List<Object[]> pacientePorCitas=citaService.pacientesPorCitas();
				model.put("pacientePorCitas", pacientePorCitas);
				return "administrador/reporteCitaPorPaciente";
			}else{
				return "error/error";
			}
		}else{
	...
	@GetMapping("/reporteAreasCitasMes")
	public String reporteAreasCitasMes(HttpSession session,Model model) {
		if(session.getAttribute("idusuario")!=null){
			Administrador admin=administradorService.findByDni(session.getAttribute("idusuario").toString());
			if(admin.getRol().equalsIgnoreCase("Admin")){
				model.addAttribute("admin",admin);
				return "administrador/reporteAreasCitasMes";
			}else{
				return "error/error";
			}
		}else{
	...
	@PostMapping("/reporteAreasCitasMes")
	public String reporteAreasCitasMes(HttpSession session,Map<String,Object> model,String mes) {
		if(session.getAttribute("idusuario")!=null){
			Administrador admin=administradorService.findByDni(session.getAttribute("idusuario").toString());
			if(admin.getRol().equalsIgnoreCase("Admin")){
				DateTimeFormatter formato = DateTimeFormatter.ofPattern("yyyy-MM");
				...
				List<Object[]> cantidadAreasMes=resultadoService.cantidadAreasMes(mes);
				model.put("cantidadAreasMes", cantidadAreasMes);
				model.put("mes", mes);
				return "administrador/reporteAreasCitasMes";
			}else{
				return "error/error";
	...
	@GetMapping("/reporteIngresosAreasCita")
	public String reporteIngresosAreasCita(HttpSession session,Model model) {
		if(session.getAttribute("idusuario")!=null){
			Administrador admin=administradorService.findByDni(session.getAttribute("idusuario").toString());
			if(admin.getRol().equalsIgnoreCase("Admin")){
				model.addAttribute("admin",admin);
				return "administrador/reporteIngresosAreasCita";
			}else{
				return "error/error";
			}
		}else{
	...
	@PostMapping("/reporteIngresosAreasCita")
	public String reporteIngresoAreasCita(HttpSession session,Map<String,Object> model,String mes) {
		if(session.getAttribute("idusuario")!=null){
			Administrador admin=administradorService.findByDni(session.getAttribute("idusuario").toString());
			if(admin.getRol().equalsIgnoreCase("Admin")){
				DateTimeFormatter formato = DateTimeFormatter.ofPattern("yyyy-MM");
				...
				List<Object[]> ingresoAreasMes=resultadoService.ingresoAreasMes(mes);
				model.put("ingresoAreasMes", ingresoAreasMes);
				model.put("mes", mes);
				return "administrador/reporteIngresosAreasCita";
			}else{
				return "error/error";
