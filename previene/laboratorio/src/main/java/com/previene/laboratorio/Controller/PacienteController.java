...
public class PacienteController {
	...
	@GetMapping("/verResultado/{id_cita}")
	public String verResultado(HttpSession session,@PathVariable Integer id_cita,Map<String,Object> model) {
		if(session.getAttribute("idusuario")!=null){
			Paciente pac=pacienteService.findByDni(session.getAttribute("idusuario").toString());
			if(pac.getRol().equalsIgnoreCase("Paciente")){
				if(id_cita>0){
